    ...
    <AppShell><div className="px-6 py-10 flex flex-col items-center">
      <div className="w-full max-w-sm">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-[20px] font-bold text-[var(--ink)] tracking-tight">Your children</h1>
          <button onClick={logout} className="text-[11px] font-bold text-[var(--muted)] active:text-red-500 transition-colors">Log out</button>
        </div>
          ...
          {allStudents.map((child, i) => (
            ...
            >
              <div>
                ...
                {child.class_grade && <p className="text-[12px] text-[var(--muted)]">{child.class_grade}</p>}
              </div>
              <span className="text-[12px] text-[var(--muted)]">🔥{child.current_streak || 0}</span>
            </motion.button>
          ))}
        </div>
        ...
        {!adding ? (
          ...
          <motion.form
            ...
            className="bg-[var(--card)] rounded-3xl p-4 border border-[var(--border)] shadow-sm space-y-2"
          >
            <input
              type="text"
              ...
              required
              className="w-full border border-[var(--border)] rounded-2xl px-3 py-2 text-[14px] text-[var(--ink)] placeholder:text-[var(--muted)] bg-[var(--card-alt)] focus:outline-none focus:ring-2 focus:ring-brand-500 focus:bg-[var(--card)] transition-colors"
            />
            <input
              type="text"
